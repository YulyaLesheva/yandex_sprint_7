...
class TestOrders:
    ...
    def test_create_order(self, colors):
        payload = TestHelper().generate_order_data()
        payload["color"] = colors

        response = TestHelper.create_order(payload)
        response_parsed_text = TestHelper.parse_response_text(response)
    ...
    def test_get_orders(self):
        response = TestHelper.get_orders()
        response_parsed_text = TestHelper.parse_response_text(response)
